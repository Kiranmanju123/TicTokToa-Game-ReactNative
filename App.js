...
export default class App extends React.Component {
  ...
  resetGame = () => {
    //reset all the values and state
  ...
  (itemArray[7] == itemArray[8]) ) {
    this.setState({winMessage:(itemArray[6] ? 'Cross' : "Circle".concat
    (' win'))})
...
(itemArray[4] == itemArray[7]) ) {
  this.setState({winMessage:(itemArray[1] ? 'Cross' : "Circle".concat
  (' win'))})


} else  if((itemArray[2] !=="empty") && (itemArray[2] == itemArray[5]) && 
(itemArray[5] == itemArray[8]) ) {
  this.setState({winMessage:(itemArray[2] ? 'Cross' : "Circle".concat
  (' win'))})
...
(itemArray[4] == itemArray[6]) ) {
  ...
  render() {
   return (
      ...
      <Text style={{fontSize:30, fontWeight:"bold",color:"green",marginBottom: 30,marginTop: 50}} > LEARN CODE ONLINE</Text>
        ...
        <Button full rounded primary style= {styles.btn}  onPress={ () => this.resetGame}>
